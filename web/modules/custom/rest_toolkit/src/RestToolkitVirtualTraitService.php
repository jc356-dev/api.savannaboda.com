...
/**
 * Class RestToolkitVirtualTraitService.
 */
class RestToolkitVirtualTraitService {
  ...
  /**
   * Constructs a new RestToolkitVirtualTraitService object.
   */
  public function __construct(SerializerInterface $serializer, RequestStack $request_stack) {
    ...
    // @todo might want this for other virtual use.
    // Could enable but check for Content disposition headers or Content Type.
   ...
   * @return     Response|array  ( description_of_the_return_value )
   */
  public function sendNormalizedResponse($response) {
    if ($this->format == 'html') {
      return $response;
    }
    else {
      ...
      switch ($fieldType) {
        ...
        case 'commerce_price':
          ...
          if (is_array($this->postData[$fieldName]) &&
            isset($this->postData[$fieldName]['number'])) {
