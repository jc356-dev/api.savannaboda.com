...
namespace Drupal\rest_toolkit\Plugin\rest\resource;
 ...
 *
 * @RestResource(
 *   id = "rt_media_file_with_mimesniff:upload",
 *   label = @Translation("Rest Toolkit Upload File as Media entity - with mimesniff"),
 *   serialization_class = "Drupal\file\Entity\File",
 *   uri_paths = {
 *     "https://www.drupal.org/link-relations/create" = "/rest_toolkit/upload/media"
 *   }
 * )
 */
class RTMediaFileWithMimesniffUploadResource extends ResourceBase {
  ...
  public function post(Request $request) {
    // These were removed, need to mimesniff and solve for:
    // $bundle, $field_name
    $entity_type_id = 'media';

    $filename = $this->validateAndParseContentDispositionHeader($request);

    // JW - mimetype guess and assume bundle, fieldname.
    // $prepared_filename1 = $this->prepareFilename($filename, []);
    $mimetype = $this->mimeTypeGuesser->guess($filename);
    if (empty($mimetype)) {
      throw new HttpException(500, 'Could not determine mimetype');
    }
    $mimetypeMapping = [
      'audio' => [
        'audio',
    ...
    $bundle = '';
    foreach ($mimetypeMapping as $bundleName => $mimePrefixes) {
      foreach ($mimePrefixes as $mimePrefix) {
        if (strstr($mimetype, $mimePrefix)) {
          $bundle = $bundleName;
          break 2;
        }
      }
    ...
    // Hopefully never happens?  They uploaded something weird.
    if (empty($bundle)) {
      throw new HttpException(500, 'Could not determine bundle from mimetype');
    }

    // Now, just assume field_name based on which media bundle it is.
    $bundleToFieldnameMapping = [
      'audio' => 'field_media_audio_file',
      'document' => 'field_media_document',
      'image' => 'field_media_image',
      'video' => 'field_media_video_file',
    ];
    $field_name = $bundleToFieldnameMapping[$bundle];
    ...
    //
    $rest_toolkit = \Drupal::service('rest_toolkit.virtual_trait');
    // Create new Media entity.
    $mediaParams = [
      'uid' => $rest_toolkit->user->id(),
      'bundle' => $bundle,
      $field_name => $file->id()
    ];

    $mediaEntity = Media::create($mediaParams);
    $mediaEntity->save();

    return $rest_toolkit->sendNormalizedResponse($mediaEntity);
